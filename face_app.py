...
def handle_file():
    ...
    if file and allowed_file(file.filename):
        img = fipu_face.get_from_file(request.files[IMG_FILE], get_response_encoding())
...
def upload_file():
    if is_file():
        ...
        img = fipu_face.get_from_base64(file64, get_response_encoding())
        return create_response(img)

    elif is_file_bytes():
        file_bytes = get_value(IMG_FILE_BYTES)
        img = fipu_face.get_from_bytes(file_bytes, get_response_encoding())
        return create_response(img)
